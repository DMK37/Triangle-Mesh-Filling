...
public class ScanLineAlgorithm {
    ...
    public static void fillTriangle(Triangle triangle, Graphics2D g, Mesh mesh) {
        ...
        if(OptionsPanel.gridBox.isSelected()) {
            g.drawLine((int) (triangle.points[0].x * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[0].y * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[1].x * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[1].y * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN));
            g.drawLine((int) (triangle.points[1].x * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[1].y * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[2].x * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[2].y * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN));
            g.drawLine((int) (triangle.points[2].x * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[2].y * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[0].x * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN), (int) (triangle.points[0].y * DrawPanel.HEIGHT + 2 * DrawPanel.MARGIN));
        }

    }


    public static List<ColoredPoint> fillTriangleAsync(Triangle triangle, Mesh mesh) {
        List<Point> points = new ArrayList<>();
                ...
                for (int x = xMin; x < xMax; x++) {
                    ...
                    if(DrawPanel.image == null)
                    {
                        ...
                        if(OptionsPanel.normalMapBox.isSelected()) {
                            float[][] mat = Calculation.calculateM(nVector);
                            // rescale rgb
                            nVector = Calculation.matrixByVector(mat, new float[] {(float) red / 255 * 2 - 1, (float) green / 255 * 2 - 1, (float) blue / 255 * 2 - 1});
                        }
                        float[] objColor = new float[] {(float) red / 255, (float) green / 255, (float) blue / 255};
                        c = triangle.pickColor(DrawPanel.lightColor, objColor, nVector, Calculation.calculateVectorFromPoints(new float[]{trX, trY, trZ}, DrawPanel.ligthPosition));
                    }
                    //g.setColor(new Color((int) (c[0] * 255), (int) (c[1] * 255), (int) (c[2] * 255)));
                    coloredPoints.add(new ColoredPoint(x,y, new Color((int) (c[0] * 255), (int) (c[1] * 255), (int) (c[2] * 255))));
                    //g.drawOval(x, y, 1, 1);
                }
            }
